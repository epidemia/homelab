...
This repository defines a Docker Compose homelab. The root [`compose.yaml`](/Users/andreimochalov/Projects/homelab/compose.yaml) aggregates service stacks from `traefik/`, `pihole-unbound/`, `pocket-id/`, `miniflux/`, `readeck/`, `beszel/`, `mazanoke/`, `bentopdf/`, and `gitea/`. Each service directory owns its own `compose.yaml`; keep service-specific networks, labels, and volumes there. Miniflux uses PostgreSQL via `miniflux-db`; Readeck uses SQLite under `/srv/docker/readeck`; Gitea stores its SQLite database, repositories, and application state under `/srv/docker/gitea`. Gitea is a private single-user service authenticated through Pocket ID OIDC, with registration and unused collaboration features disabled. The [`restic/`](/Users/andreimochalov/Projects/homelab/restic) directory contains backup automation, systemd units, and restore documentation. Use [`.env.example`](/Users/andreimochalov/Projects/homelab/.env.example) as the template for local `.env`.
